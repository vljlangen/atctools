...
### atctools::validate_atc_by_reference()

The `validate_atc_by_reference()` function checks if each ATC code in a specified column exists in a reference list of valid codes (e.g., from `example_reference_data`). This is useful for ensuring codes are not just structurally valid but also known and accepted.
