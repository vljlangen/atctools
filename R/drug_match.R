drug_match <- function(input_data, drug_column, reference_data) {
  ...
  # Initialize new columns for ATC and hila
  input_data[[paste0(drug_column, "_ATC")]] <- NA
  input_data[[paste0(drug_column, "_hila")]] <- NA
  ...
  for (i in 1:nrow(input_data)) {
    ...
    if (is.na(original_value)) {
      ...
      input_data[[paste0(drug_column, "_hila")]][i] <- NA
    } else {
      ...
      input_data[[paste0(drug_column, "_hila")]][i] <- closest_match
    }

    # Create flag variable
    flag <- ifelse(substr(original_value, 1, 3) != substr(input_data[[paste0(drug_column, "_hila")]][i], 1, 3), 1, 0)
